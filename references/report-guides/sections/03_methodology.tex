...
\subsubsection{Phân tích Cơ chế Thiếu Dữ liệu}
...
\begin{itemize}
    ...
    \item \textbf{Số lượng đặc trưng:} 64 cột sau feature engineering.
\end{itemize}
...
\subsection{Kỹ thuật Đặc trưng Miền Thờ gian}
...
Dữ liệu khí tượng từ Open-Meteo API (tọa độ 34,94°N, 127,70°E)
...
\begin{equation}
\min_G \max_D \mathcal{L}(D,G) = \mathbb{E}_{x\sim p_{\text{data}}}\big[\ln D(x)\big] +
\mathbb{E}_{z\sim p_z}\big[\ln(1-D(G(z)))\big]
\label{eq:gan}
\end{equation}
...
\STATE \COMMENT{Idling, Leakage, Overload with confidence scores}
\STATE $\text{run\_pipeline\_assertions}(D_7, \text{stage}=\text{post\_labeling})$
\IF{use\_GAN}
    \STATE $G, D_{\text{net}} \leftarrow \text{train\_GAN}(D_7^{(\text{normal})})$
