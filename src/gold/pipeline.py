...
from src.bronze.data_loader import load_steel_data, clean_data, inspect_data
...
class ElectrimightPipeline:
    ...
    def run_cleaning(self) -> pd.DataFrame:
        ...
        save_data(self.clean_df, SILVER_DIR / "steel_clean.csv")
    ...
    def run_gan_augmentation(self) -> pd.DataFrame:
        ...
        Chiến lược: Huấn luyện GAN trên lớp thiểu số (anomaly) để học
        phân phối của các mẫu bất thường, sau đó sinh synthetic samples
        bổ sung vào tập dữ liệu gốc nhằm giảm mất cân bằng lớp.
        """
        self.logger.info("=" * 60)
        self.logger.info("STEP 7: GAN AUGMENTATION")
        self.logger.info("=" * 60)

        from src.gold.gan_augmentation import train_gan, generate_synthetic_samples
        from sklearn.preprocessing import MinMaxScaler

        # Chỉ huấn luyện GAN trên lớp thiểu số (anomaly) để tăng cường minority
        minority_df = self.labeled_df[self.labeled_df["anomaly_any"] == True].copy()
        if minority_df.empty:
            self.logger.warning("Không có mẫu anomaly nào — bỏ qua GAN augmentation.")
            self.final_df = self.labeled_df
            return self.final_df

        numeric_cols = minority_df.select_dtypes(include=[np.number]).columns.tolist()
        data_array = minority_df[numeric_cols].dropna().values
        ...
        generator, _ = train_gan(
            ...
            batch_size=min(64, len(data_array)),
        )
