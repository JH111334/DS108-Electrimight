# Labeling Guideline for Industrial Electricity Proxy Anomaly Detection

Tài liệu này định nghĩa các quy tắc và nguyên tắc gán nhãn bất thường dạng proxy
trên dữ liệu tiêu thụ điện năng công nghiệp. Bộ dữ liệu mặc định là UCI Steel,
nhưng codebase hiện có schema contract để mở rộng sang meter data công nghiệp
hoặc nông nghiệp nếu map được timestamp, active energy/power, reactive power
và power factor.
...
|-----------|-------|-------|
| Load_Type == "Light_Load" hoặc low-load proxy | Thiết bị đang ở chế độ tải nhẹ hoặc không tải; nếu dataset mới không có `Load_Type`, dùng phân vị thấp của `Usage_kWh` làm proxy | Dataset classification / fallback từ meter data |
...
| PF_lag < 0.50, hoặc PF_lead nếu không có PF_lag | Hệ số công suất cực kỳ thấp - dấu hiệu idling | IEEE 519 severe threshold |

#### Confidence Score

```
score = 0.3 * I(Light_Load hoặc low-load proxy)
      + 0.3 * I(Off-hours)
      + 0.2 * I(Usage_kWh > median)
      + 0.2 * I(effective_PF < 0.50)
```
...
| reactive_magnitude > percentile 99.5_Q | Công suất phản kháng trễ/sớm hoặc tổng phản kháng cao bất thường | Domain knowledge |
| effective_PF < 0.70 | PF suy giảm rõ rệt dưới mức chấp nhận | IEEE 519 minimum |

Logic: `Usage_cao AND (Reactive_cao OR PF_thấp)`, trong đó reactive magnitude có thể lấy từ `Q_lagging`, `Q_leading`, `Q_net` hoặc `Q_total` nếu có.
