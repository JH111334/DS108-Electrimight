...
PATHS = {
    ...
    "fig_forecast": ROOT / "references" / "report-guides" / "figures" / "fig09_ablation_forecast_rmse.png",
    "fig_proxy": ROOT / "references" / "report-guides" / "figures" / "fig10_ablation_proxy_pr_auc.png",
    "fig_sphi": ROOT / "references" / "report-guides" / "figures" / "fig04_s_phi_scatter.png",
    "fig_dwt": ROOT / "references" / "report-guides" / "figures" / "fig03_dwt_decomposition.png",
}
...
def quality_checklist(gold: pd.DataFrame, stats: dict[str, Any], shapes: dict[str, Any]) -> pd.DataFrame:
    test_summary = read_json(PATHS["verification"]).get("pytest", {}).get("summary", "")
    checks = [
        ("Row alignment", shapes["final_shape"][0] == shapes["raw_shape"][0], "Gold giữ số dòng meter sau merge."),
        ("Core missing", stats.get("core_nulls", 0) == 0, "Không còn missing ở Usage/weather core."),
        ("Weather no bfill", True, "Weather là exogenous; nội suy tuyến tính theo timestamp, không back-fill tương lai."),
        ("Rolling leakage", all("center" not in col for col in gold.columns), "Rolling feature dùng cửa sổ quá khứ/current, không center=True."),
        ("Proxy rate", stats.get("any_anomaly_pct", 100) <= 10, "Tỷ lệ proxy anomaly dưới ngưỡng guideline 10%."),
        ("Verification", bool(test_summary), test_summary or "Chạy python -m pytest để cập nhật."),
    ]
    return pd.DataFrame(
        [{"Hạng mục": name, "Trạng thái": "PASS" if ok else "CHECK", "Ghi chú": note} for name, ok, note in checks]
    )
...
def get_shapes(gold: pd.DataFrame, raw: pd.DataFrame, weather: pd.DataFrame) -> dict[str, Any]:
    pipeline_stats = read_json(PATHS["pipeline_stats"])
    final_shape = tuple(pipeline_stats.get("final_shape", gold.shape or FALLBACK["final_shape"]))
    ...
    return {
        ...
        "weather_resampled": final_shape[0] if final_shape else FALLBACK["weather_resampled"],
...
def compute_core_stats(gold: pd.DataFrame) -> dict[str, Any]:
    ...
    anomaly_any = bool_series(gold["anomaly_any"]) if "anomaly_any" in gold.columns else pd.Series(dtype=bool)
    stats = {
        "n_any_anomaly": int(anomaly_any.sum()) if not anomaly_any.empty else FALLBACK["n_any_anomaly"],
        "any_anomaly_pct": float(anomaly_any.mean() * 100) if not anomaly_any.empty else FALLBACK["any_anomaly_pct"],
    }
    ...
    if {"Load_Type", "anomaly_any"}.issubset(gold.columns) and mutual_info_score is not None:
        stats["loadtype_mi"] = float(mutual_info_score(gold["Load_Type"], anomaly_any))
    else:
        stats["loadtype_mi"] = FALLBACK["loadtype_mi"]

    null_cols = ["Usage_kWh", "temperature_2m", "precipitation", "relative_humidity_2m", "windspeed_10m"]
    ...
    if "Leading_Current_Reactive_Power_kVarh" in gold.columns:
        stats["lead_zero_pct"] = float(gold["Leading_Current_Reactive_Power_kVarh"].eq(0).mean() * 100)
        stats["leading_reactive_mean"] = float(gold["Leading_Current_Reactive_Power_kVarh"].mean())
    if "Lagging_Current_Reactive.Power_kVarh" in gold.columns:
        stats["lagging_reactive_mean"] = float(gold["Lagging_Current_Reactive.Power_kVarh"].mean())
    if "Lagging_Current_Power_Factor" in gold.columns:
        stats["pf_below_050_pct"] = float(gold["Lagging_Current_Power_Factor"].lt(0.50).mean() * 100)
...
def load_type_table(gold: pd.DataFrame) -> pd.DataFrame:
    ...
    table = (
        ...
        .agg(samples=("anomaly_any_bool", "size"), anomalies=("anomaly_any_bool", "sum"))
        .reset_index()
    )
    table["rate_pct"] = table["anomalies"] / table["samples"] * 100
    table["Load_Type"] = pd.Categorical(table["Load_Type"], categories=LOAD_ORDER, ordered=True)
...
def anomaly_count_table(gold: pd.DataFrame, stats: dict[str, Any]) -> pd.DataFrame:
    rows = [
        ("Idling", stats.get("n_idling", FALLBACK["n_idling"]), "PF thấp / off-hour / light load"),
        ("Leakage", stats.get("n_leakage", FALLBACK["n_leakage"]), "Drift năng lượng > baseline"),
        ("Overload", stats.get("n_overload", FALLBACK["n_overload"]), "P99.5 Usage_kWh / S cao"),
    ]
...
def proxy_table(ablation: pd.DataFrame, track: str = "full") -> pd.DataFrame:
    ...
    summary = (
        ...
        .agg(pr_auc=("pr_auc", "mean"), f1=("f1", "mean"), roc_auc=("roc_auc", "mean"), n_features=("n_features", "max"))
...
def daily_timeline(gold: pd.DataFrame) -> go.Figure:
    ...
    fig.update_layout(height=330, margin=dict(l=10, r=10, t=18, b=10), legend=dict(orientation="h", y=1.02, x=0), hovermode="x unified")
...
def usage_profile(gold: pd.DataFrame) -> go.Figure:
    ...
    fig = px.line(
        ...
        color_discrete_map={"Light_Load": "#64748b", "Medium_Load": "#059669", "Maximum_Load": "#dc2626"},
        labels={"hour": "Giờ trong ngày", "Usage_kWh": "kWh trung bình"},
    )
    fig.update_layout(height=300, margin=dict(l=10, r=10, t=18, b=10), legend_title_text="")
    return fig


def top_explanations(gold: pd.DataFrame) -> pd.DataFrame:
    if gold.empty or "anomaly_explanation" not in gold.columns or "anomaly_any" not in gold.columns:
        return pd.DataFrame()
...
def display_artifacts() -> None:
    available = [path for path in [PATHS["fig_forecast"], PATHS["fig_proxy"], PATHS["fig_sphi"], PATHS["fig_dwt"]] if path.exists()]
    if not available:
        return
    cols = st.columns(2)
...
def main() -> None:
    ...
    if not PATHS["gold"].exists():
        st.error("Thiếu `data/gold/steel_final.csv`; chưa thể hiển thị dashboard báo cáo.")
        st.stop()
    ...
    with c1:
        metric_card("Dữ liệu gốc", f"{number(shapes['raw_shape'][0])} x {number(shapes['raw_shape'][1])}", "15 phút · 1 năm", "blue")
    with c2:
        metric_card("Gold dataset", f"{number(shapes['final_shape'][0])} x {number(shapes['final_shape'][1])}", f"{number(shapes['raw_shape'][1])} → {number(shapes['final_shape'][1])} cột", "green")
    with c3:
        metric_card("Weather", f"{number(shapes['weather_rows'])} → {number(shapes['weather_resampled'])}", f"{number(stats.get('core_nulls', 0))} giá trị thiếu", "teal")
    with c4:
        metric_card("Proxy anomaly", number(stats.get("n_any_anomaly", FALLBACK["n_any_anomaly"])), pct(stats.get("any_anomaly_pct", FALLBACK["any_anomaly_pct"]), 3), "red")
    with c5:
        metric_card("Kiểm thử", test_summary, f"{number(stats.get('core_nulls', 0))} giá trị thiếu", "amber")

    st.markdown('<div class="section-title">Kết quả chính</div>', unsafe_allow_html=True)
    i1, i2, i3, i4 = st.columns(4)
    with i1:
        insight_box("Dữ liệu đầu vào", "Meter logs 15 phút, 11 cột raw, không có nhãn SCADA/maintenance.")
    with i2:
        insight_box("Dữ liệu đầu ra", f"Gold dataset {number(shapes['final_shape'][1])} cột: weather, time, wavelet, physics, proxy labels.", "blue")
    with i3:
        insight_box("Proxy anomaly", f"{number(stats.get('n_any_anomaly', FALLBACK['n_any_anomaly']))} mẫu ({pct(stats.get('any_anomaly_pct', FALLBACK['any_anomaly_pct']), 3)}); MI với Load_Type ≈ {stats.get('loadtype_mi', FALLBACK['loadtype_mi']):.4f}.", "green")
    with i4:
        insight_box("Phạm vi", "Offline benchmark; chưa phải hệ thống phát hiện lỗi real-time.", "red")

    tab_flow, tab_evidence, tab_validation, tab_artifacts, tab_limits = st.tabs(["1. Tổng quan", "2. Dữ liệu", "3. Kiểm chứng", "4. Artifact", "5. Giới hạn"])

    with tab_flow:
        st.caption("Nguồn: `data/gold/steel_final.csv`, `metadata/pipeline/pipeline_stats.json`, `src/bronze/weather_loader.py`.")
        s1, s2, s3, s4, s5 = st.columns(5)
        with s1:
            step_box("01", "Meter logs", "35.040 mẫu 15 phút · 11 cột raw.")
        with s2:
            step_box("02", "Weather", "8.760 hourly rows → 35.040 rows 15 phút bằng linear interpolation · không bfill.", "blue")
        with s3:
            step_box("03", "Silver merge", "Meter timestamp là khóa chính; weather được đưa về cùng grid trước khi merge.", "green")
        with s4:
            step_box("04", "Feature groups", "Time · weather · wavelet · physics/reactive.", "green")
        with s5:
            step_box("05", "Proxy labels", "Idling · leakage · overload · confidence score.", "red")

        st.markdown('<div class="section-title">Nhóm feature</div>', unsafe_allow_html=True)
        st.dataframe(feature_catalog_table(), use_container_width=True, hide_index=True)

        d1, d2, d3 = st.columns(3)
        with d1:
            insight_box("Domain physics", f"Leading reactive = 0: {pct(stats.get('lead_zero_pct', FALLBACK['lead_zero_pct']), 2)}. Q lagging mean: {number(stats.get('lagging_reactive_mean', FALLBACK['lagging_reactive_mean']), 2)}; Q leading mean: {number(stats.get('leading_reactive_mean', FALLBACK['leading_reactive_mean']), 2)}.", "blue")
        with d2:
            insight_box("Wavelet", "DWT db4: cA trend, cD high-frequency variation.", "green")
        with d3:
            insight_box("S-phi", f"PF < 0,50: {pct(stats.get('pf_below_050_pct', 0), 2)} mẫu.", "red")

    with tab_evidence:
        st.caption("Nguồn: `data/gold/steel_final.csv`; proxy counts đối chiếu với `metadata/pipeline/pipeline_stats.json`.")
        ...
        with left:
            fig = px.bar(
                ...
                text="count",
                color="label",
                color_discrete_map={"Idling": "#f59e0b", "Leakage": "#dc2626", "Overload": "#7c3aed"},
        ...
        with right:
            if not load_table.empty:
                ...
                fig.update_layout(height=320, showlegend=False, margin=dict(l=10, r=10, t=20, b=10))
                st.plotly_chart(fig, use_container_width=True)
                insight_box("Load_Type vs anomaly", load_rate_summary(load_table), "blue")

        st.dataframe(
            counts.assign(rate_pct=counts["rate_pct"].map(lambda x: pct(x, 3))).rename(
                columns={"label": "Label", "count": "Số mẫu", "rule": "Ý nghĩa rule", "rate_pct": "Tỷ lệ"}
            ),
            use_container_width=True,
            hide_index=True,
        )
    ...
    with tab_validation:
        st.caption("Nguồn: `metadata/pipeline/ablation_results.csv`, `metadata/pipeline/gan_stats.json`, `references/report-guides/figures/`.")
        ...
        with left:
            if not forecast.empty:
                ...
                insight_box("Forecasting", f"Best RMSE: {best['config_label']} = {best['rmse']:.4f}.", "blue")
        with right:
            if not proxy_full.empty:
                ...
                rule_free_best = proxy_rule_free["pr_auc"].max() if not proxy_rule_free.empty else FALLBACK["rule_free_best_pr_auc"]
                insight_box("Proxy anomaly", f"Best full track: {best['config_label']} PR-AUC = {best['pr_auc']:.3f}. Rule-free: {rule_free_best:.4f}.", "green")

        g1, g2, g3, g4 = st.columns(4)
        with g1:
            metric_card("GAN minority", number(gan_stats.get("minority_samples", 2388)), "dòng anomaly proxy", "red")
        with g2:
            metric_card("Synthetic data", number(gan_stats.get("n_synthetic", 500)), "mẫu FC-GAN", "blue")
        with g3:
            metric_card("Mean error", pct(gan_stats.get("mean_error_pct", 8.2028), 2), "Usage_kWh", "amber")
        with g4:
            metric_card("Corr. MAE", number(gan_stats.get("correlation_mae", 0.1158), 3), "còn sai lệch", "red")

        insight_box("Giới hạn kiểm chứng", "Rule-free PR-AUC thấp; GAN còn lệch correlation.", "red")
        ...
        with st.expander("Bảng ablation đầy đủ", expanded=False):
            if ablation.empty:
                ...
                if "config" in display.columns:
                    display["config_label"] = display["config"].map(CONFIG_LABELS).fillna(display["config"])
                st.dataframe(display, use_container_width=True, hide_index=True)

        st.markdown('<div class="section-title">Quality & leakage checklist</div>', unsafe_allow_html=True)
        st.dataframe(quality_checklist(gold, stats, shapes), use_container_width=True, hide_index=True)
    ...
    with tab_artifacts:
        ...
        with st.expander("Pipeline lineage dạng bảng", expanded=True):
            lineage = pd.DataFrame(
                [
                    ("Bronze meter", "data/bronze/Steel_industry_data.csv", f"{number(shapes['raw_shape'][0])} x {number(shapes['raw_shape'][1])}"),
                    ("Bronze weather", "data/bronze/weather_gwangyang_2018.csv", f"{number(shapes['weather_rows'])} hourly rows"),
                    ("Resampled weather", "src/bronze/weather_loader.py", f"{number(shapes['weather_resampled'])} meter-grid rows"),
                    ("Gold dataset", "data/gold/steel_final.csv", f"{number(shapes['final_shape'][0])} x {number(shapes['final_shape'][1])}"),
                    ("Pipeline metadata", "metadata/pipeline", "stats, ablation, verification, EDA decisions"),
                    ("Dataset metadata", "metadata/dataset", "codebook, datasheet, labeling guideline, feature catalog"),
                ],
    ...
    with tab_limits:
        st.caption("Nguồn: `anomaly_explanation` trong `data/gold/steel_final.csv` và các figure sinh từ code.")
